...
#   temps are now in 14.2 fixed point notation
#   (i.e. measured in quarter-degrees)
#   temps are not permitted to be negative (BUG:may result in numtemps fewer
#       than requested)
#   bugfix: --num-temps command line option works.
#    2012-11-08, DaveX: Modified to add  --vcc=, --min_adc=, --mult and to
#       print per-ADC comments

"""
    Thermistor Value Lookup Table Generator
    Generates lookup to temperature values for use in a microcontroller in 
    C format based on: 
    http://hydraraptor.blogspot.com/2007/10/measuring-temperature-easy-way.html
    The main use is for Arduino programs that read data from the circuit
    board described here:
        http://make.rrrf.org/ts-1.0
        or
        http://reprap.org/wiki/Temperature_Sensor_2_0
    Usage: python createTemperatureLookup.py [options]
    Options:
        -h, --help          show this help
        --r0=... 		    thermistor rating where # is the ohm rating
            of the thermistor at t0 (eg: 10K = 10000)
        --t0=... 		    thermistor temp rating where # is the temperature in
            Celsuis to get r0 (from your datasheet)
        --beta=...		    thermistor beta rating. 
            see http://reprap.org/bin/view/Main/MeasuringThermistorBeta
        --r1=...		    R1 rating where # is the ohm rating of
            R1 (eg: 10K = 10000)
        --r2=...    	    R2 rating where # is the ohm rating of
            R2 (eg: 10K = 10000)
        --num-temps=... 	the number of temperature points to calculate
            (default: 20)
        --max-adc=...    	the max ADC reading to use.  if you use R1,
            it limits the top value for the thermistor circuit, and thus
            the possible range of ADC values
    ...
    It is suggested to generate more values than you need, and delete some
        of the ones in the ranges that aren't interesting. This will
        improve accuracy in the temperature ranges that are important
        to you.
"""
...
class Thermistor:
    ...
    def __init__(self, r0, t0, beta, r1, r2, vcc, vadc):
        ...
        self.t0 = t0 + 273.15       # temp. at stated resistance, e.g. 25C
...
def main(argv):
    ...
    adcs.append(max_adc)
#  adcs = [1, 20, 25, 30, 35, 40, 45, 50, 60, 70, 80, 90, 100, 110, 130, 150,
#  190, 220,  250, 300]
#  Chop of negative temperatures (as we're using a unsigned 16-bit value
# for temp)
    for i in range(0, len(adcs)):
        if int(t.temp(adcs[i])*mult) < 0:
            ...
            # If the closes ADC reading to 0C is negative,
            # convert to next highest ADC reading
            if int(t.temp(adcs[i])*mult) < 0:
                adcs[i] -= 1
            break
    ...
    print("// Made with createTemperatureLookup.py "
          "(https://github.com/traumflug/Teacup_Firmware/blob/master/createTemperatureLookup.py)")
    ...
    print(
        "// ADC counts from {min} to {max} by {x}".format(min=min_adc, max=max_adc, x=increment))
    if table == True:
        print(
        ...
        print("{ //" + " Table 0 chunk for B={b}, R0={r0}, R1={r1}, R2={r2}, Vref={v}".format(
    ...
    for adc in adcs:
        ...
        resolution = (t.temp(adc-1)-t.temp(adc) if adc >
                      1 else t.temp(adc) - t.temp(adc+1))
        sep = (',' if counter != len(adcs) else ' ')
        print("   {%4s, %6s}%s // %7.2f C,  %7.0f Ohm, %0.3f V, %0.2f C/count, %0.2fmW" %
              (adc, int(t.temp(adc)*mult), sep, degC, resistance, vTherm, resolution, ptherm*1000))
    if table == False:
        print("};")
    else:
