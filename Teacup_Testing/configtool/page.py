...
class Page(object):
  ...
  def enableAll(self, flag = True):
    for c in list(self.textControls.keys()):
      self.textControls[c].Enable(flag)
    for c in list(self.checkBoxes.keys()):
      self.checkBoxes[c].Enable(flag)
    for c in list(self.radioButtons.keys()):
      self.radioButtons[c].Enable(flag)
    for c in list(self.choices.keys()):
  ...
  def addCheckBox(self, name, validator):
    if name in list(self.labels.keys()):
  ...
  def setChoice(self, name, cfgValues, default):
    if name in list(cfgValues.keys()) and cfgValues[name][1] == True:
      bv = cfgValues[name][0]
    else:
      bv = default
  ...
  def setHelpText(self, ht):
    for k in list(self.textControls.keys()):
      if k in list(ht.keys()):
        self.textControls[k].SetToolTip(ht[k])

    for k in list(self.checkBoxes.keys()):
      if k in list(ht.keys()):
        self.checkBoxes[k].SetToolTip(ht[k])

    for k in list(self.radioButtons.keys()):
      if k in list(ht.keys()):
        self.radioButtons[k].SetToolTip(ht[k])
        if k in list(self.radioButtonBoxes.keys()):
          self.radioButtonBoxes[k].SetToolTip(ht[k])

    for k in list(self.choices.keys()):
      if k in list(ht.keys()):
        self.choices[k].SetToolTip(ht[k])

    for k in list(self.boolChoices.keys()):
      for candidate in list(ht.keys()):
  ...
  def insertValues(self, cfgValues):
    ...
    self.enableAll(True)
    for k in list(self.fieldValid.keys()):
      self.fieldValid[k] = True

    for k in list(self.checkBoxes.keys()):
      if k in list(cfgValues.keys()) and cfgValues[k]:
        self.checkBoxes[k].SetValue(True)
      else:
        self.checkBoxes[k].SetValue(False)

    for k in list(self.textControls.keys()):
      if k in list(cfgValues.keys()):
    ...
    for k in list(self.boolChoices.keys()):
      ...
      for cfg in list(cfgValues.keys()):
        if cfg.startswith(k):
          if cfg in list(self.labels.keys()):
  ...
  def getValues(self):
    ...
    for k in list(self.textControls.keys()):
      ...
      if v == "":
        if k in list(self.textControlsOriginal.keys()):
    ...
    for k in list(self.choices.keys()):
      ...
      if s == "-":
        if k in list(self.choicesOriginal.keys()):
