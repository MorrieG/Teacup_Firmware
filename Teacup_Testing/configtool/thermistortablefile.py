...
class ThermistorTableFile(object):
...
def BetaTable(ofp, params, names, settings, finalTable):
  ...
  for i in samples:
    ...
    vTherm = old_div(i * vadc, 1024)
    ptherm = old_div(vTherm * vTherm, r)
    ...
    delta = old_div((t - thrm.temp(prev)), (prev - i)) if i != prev else 0
...
def after(lookup, value):
  return min([x for x in list(lookup.keys()) if x > value])

def before(lookup, value):
  return max([x for x in list(lookup.keys()) if x < value])
...
def LinearTableEstimate(lookup, value):
  ...
  y0 = lookup[x0]
  y1 = lookup[x1]
  return old_div(((value - x0) * y1 + (x1 - value) * y0), (x1 - x0))
