...
class Board(object):
  ...
  def getCPUInfo(self):
    vF_CPU = None
    if 'F_CPU' in list(self.cfgValues.keys()):
      vF_CPU = self.cfgValues['F_CPU'][0]

    vCPU = None
    if 'CPU' in list(self.cfgValues.keys()):
  ...
  def loadConfigFile(self, fn):
    ...
    for k in list(self.cfgValues.keys()):
    ...
    for k in range(len(self.sensors)):
      ...
      if tn in list(self.tempTables.keys()):
  ...
  def saveConfigFile(self, path, values):
    ...
    for ln in self.cfgBuffer:
      ...
      if m:
        t = m.groups()
        if len(t) == 2 and t[0] in list(values.keys()):
          v = values[t[0]]
      ...
      if m:
        t = m.groups()
        if len(t) == 1 and t[0] in list(values.keys()):
          v = values[t[0]]
          self.cfgValues[t[0]] = v
          if v == "" or v == False:
            fp.write("//")
