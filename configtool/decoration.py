...
class Decoration(object):
    ...
    def onPaintBackground(self, evt):
        ...
        if dc:
            # Now draw the background picture with pseudo-transparency.
            # This is, each background is drawn with the same picture,
            # without transparency, and offsetted just right to have all
            #  backgrounds in the same position relative to the *toplevel*
            # window, not relative to the current subwindow as usual.
            # Align bottom right.
            offX, offY = topLevel.GetClientSize() - self.backPic.GetSize() + \
                self.backPicOffset
            if client != topLevel:
                # Note: trying to figure this additional offset via various
                #       .GetScreenPosition() or .GetPosition() or whatever is
                #  hopeless. Of many many tries only this worked on Linux.
                offX, offY = \
                    client.ScreenToClient(
